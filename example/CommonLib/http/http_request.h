...
public:
    FCHttpRequest (int task_id, HWND notify_wnd, const HTTP_REQUEST_HEADER& request_header)
    {
        ...
        m_header.GetUrlComponent(m_tmp_host, m_tmp_object, m_tmp_port, m_tmp_scheme);
    ...
    /// Get request url.
    std::string GetURL() const {return m_header.m_url;}
...
private:
    ...
    std::string   m_tmp_host ; // parsed for thread
    std::string   m_tmp_object ;
    int m_tmp_port;
...
private:
    ...
    static DWORD WINAPI http_connect_proc (LPVOID lpParameter)
    {
        ...
        p->m_connect.Attach (InternetConnectA (p->m_session, p->m_tmp_host.c_str(), nPort, NULL, NULL, INTERNET_SERVICE_HTTP, 0, 0)) ;
    ...
    static void http_send_request_proc (FCHttpRequest* p)
    {
        ...
        // range header
        std::string   th = p->m_header.m_header ;
        if (p->m_header.m_start)
        {
            th.append (("Range: bytes=%d-\r\n"), p->m_header.m_start) ;
        }
        HttpSendRequestA (p->m_request, th.c_str(), th.size(), nSize ? pPost : NULL, nSize) ;
    }
...
private:
    ...
    {
        HINTERNET   hSession ;
        if (m_header.m_proxy_ip.size())
        {
            std::string   s ;
            s.append(StrFormat("%s:%d", m_header.m_proxy_ip.c_str(), m_header.m_proxy_port));
            hSession = InternetOpenA (m_header.m_user_agent.c_str(), INTERNET_OPEN_TYPE_PROXY, s.c_str(), NULL, 0) ;
        }
        else
        {
            hSession = InternetOpenA (m_header.m_user_agent.c_str(), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0) ;
        }

        // set proxy username and password
        if (m_header.m_proxy_username.size())
            InternetSetOptionA (hSession, INTERNET_OPTION_PROXY_USERNAME, (LPVOID)(LPCSTR)m_header.m_proxy_username.c_str(), m_header.m_proxy_username.size()) ;
        if (m_header.m_proxy_password.size())
            InternetSetOptionA (hSession, INTERNET_OPTION_PROXY_PASSWORD, (LPVOID)(LPCSTR)m_header.m_proxy_password.c_str(), m_header.m_proxy_password.size()) ;
    ...
    void OnConnectThreadFinish()
    {
        ...
        m_request.Attach (HttpOpenRequestA (m_connect, m_header.GetVerbText().c_str(), m_tmp_object.c_str(), NULL, NULL, NULL, m_header.m_open_flag, 0)) ;
